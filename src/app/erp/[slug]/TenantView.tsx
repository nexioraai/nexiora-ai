...
import { ChevronRight, Layers, Loader2, LayoutDashboard } from 'lucide-react'
...
function instanceTitle(data: Record<string, any>, moduleName: string): string {
  ...
  return txt ? String(data[txt]) : 'Unite'
}

const INTERNAL_GROUP = /(classe|class|salle|room|rayon|aisle|section|departement|department|service|niveau|grade|groupe|group|bin|etagere|shelf)/i
const AUTONOMOUS_UNIT = /(ecole|school|magasin|store|boutique|shop|hopital|hospital|clinique|clinic|filiale|subsidiary|agence|agency|succursale|branch|etablissement|establishment|entrepot|warehouse|campus|antenne)/i

export function resolveTenant(bp: any, modules: Mod[]): Tenant | null {
  if (bp?.tenant && bp.tenant.module && !INTERNAL_GROUP.test(bp.tenant.module)) {
    return bp.tenant
  }
  let best: { mod: string; key: string; score: number } | null = null
  for (const m of modules) {
    if (INTERNAL_GROUP.test(m.name)) continue
    if (!AUTONOMOUS_UNIT.test(m.name)) continue
    const fieldNames = (m.fields || []).map((f: any) => (typeof f === 'string' ? f : f?.name || ''))
    const singular = m.name.replace(/s$/i, '')
    const idRe = new RegExp('^' + singular + '_id$', 'i')
    const idField = fieldNames.find((fn: string) => idRe.test(fn) || (/_id$/i.test(fn) && fn.toLowerCase().startsWith(singular.toLowerCase().slice(0, 4))))
    if (!idField) continue
    let score = 0
    ...
    if (score >= 1 && (!best || score > best.score)) best = { mod: m.name, key: idField, score }
  }
    ...
    return (
      <div>
        <button onClick={() => setSelected(null)} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'transparent', border: 'none', color: C.muted, fontSize: 13, cursor: 'pointer', marginBottom: 16, padding: 0 }}>
          <ChevronRight size={15} style={{ transform: 'rotate(180deg)' }} /> Toutes les unites
        </button>
  ...
  return (
    <div>
      <div style={{ fontSize: 12.5, color: C.faint, marginBottom: 16 }}>
        Chaque {lbl(tenant.module).replace(/s$/i, '').toLowerCase()} est un espace autonome{tenant.manager ? ' gere par son ' + tenant.manager.toLowerCase() : ''}.
      </div>
      {loading && (
