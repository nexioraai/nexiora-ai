...
export default function ProductModal({ product: p, primary, lang = 'en', theme = 'editorial', onClose, isPodCustom = false }: Props) {
  ...
  return (
    ...
    >
      <style>{'@keyframes modalFadeIn { from { opacity: 0 } to { opacity: 1 } } @keyframes modalSlideUp { from { transform: translateY(20px); opacity: 0 } to { transform: translateY(0); opacity: 1 } }'}</style>
      <div
