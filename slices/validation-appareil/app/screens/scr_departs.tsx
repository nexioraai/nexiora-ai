...
import { screenData } from "./scr_departs.data";

export default function ScrDepartsScreen({ route }: AirScreenProps) {
  const insets = useSafeAreaInsets();
  return (
    <ScreenShell testID="scr_departs" title={screenData.title}>
      <View style={{ flex: 1, paddingBottom: insets.bottom }}>
        <AirHeader screen={screenData} blockId="blk_departs_header" />
        <AirList screen={screenData} blockId="blk_departs_liste" itemId={route?.params?.itemId} />
        <AirEmptyState screen={screenData} blockId="blk_departs_vide" />
      </View>
      <PrimaryNav destinations={primaryNav} currentScreenId="scr_departs" />
    </ScreenShell>
  );
}
