...
import { screenData } from "./scr_billets.data";

export default function ScrBilletsScreen({ route }: AirScreenProps) {
  const insets = useSafeAreaInsets();
  return (
    <ScreenShell testID="scr_billets" title={screenData.title}>
      <View style={{ flex: 1, paddingBottom: insets.bottom }}>
        <AirHeader screen={screenData} blockId="blk_billets_header" />
        <AirList screen={screenData} blockId="blk_billets_liste" itemId={route?.params?.itemId} />
        <AirEmptyState screen={screenData} blockId="blk_billets_vide" />
      </View>
      <PrimaryNav destinations={primaryNav} currentScreenId="scr_billets" />
    </ScreenShell>
  );
}
