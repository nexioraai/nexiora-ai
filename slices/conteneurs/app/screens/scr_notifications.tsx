...
import { screenData } from "./scr_notifications.data";

export default function ScrNotificationsScreen() {
  const insets = useSafeAreaInsets();
  return (
    <ScreenShell testID="scr_notifications" title={screenData.title}>
      <View style={{ flex: 1, paddingBottom: insets.bottom }}>
        <AirHeader screen={screenData} blockId="blk_alertes_header" />
        <AirList screen={screenData} blockId="blk_alertes_liste" />
        <AirEmptyState screen={screenData} blockId="blk_alertes_vide" />
        <AirButton screen={screenData} blockId="blk_alertes_tout_lu" />
      </View>
    </ScreenShell>
  );
}
