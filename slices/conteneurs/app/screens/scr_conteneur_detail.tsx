...
import { AirButton, AirDetailHeader, AirList } from "../lib/runtime/air-runtime";
import type { AirScreenProps } from "../lib/runtime/air-runtime";
import { screenData } from "./scr_conteneur_detail.data";

export default function ScrConteneurDetailScreen({ route }: AirScreenProps) {
  const insets = useSafeAreaInsets();
  return (
    <ScreenShell testID="scr_conteneur_detail" title={screenData.title}>
      <View style={{ flex: 1, paddingBottom: insets.bottom }}>
        <AirDetailHeader screen={screenData} blockId="blk_detail_entete" itemId={route?.params?.itemId} />
        <AirList screen={screenData} blockId="blk_detail_alertes" />
        <AirButton screen={screenData} blockId="blk_detail_notifications" />
        <AirButton screen={screenData} blockId="blk_detail_partager" />
      </View>
    </ScreenShell>
  );
}
