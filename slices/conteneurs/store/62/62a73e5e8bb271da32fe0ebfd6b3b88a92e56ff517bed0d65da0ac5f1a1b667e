...
import { screenData } from "./scr_conteneurs.data";

export default function ScrConteneursScreen() {
  const insets = useSafeAreaInsets();
  return (
    <ScreenShell testID="scr_conteneurs" title={screenData.title}>
      <View style={{ flex: 1, paddingBottom: insets.bottom }}>
        <AirHeader screen={screenData} blockId="blk_conteneurs_header" />
        <AirList screen={screenData} blockId="blk_conteneurs_liste" />
        <AirEmptyState screen={screenData} blockId="blk_conteneurs_vide" />
        <AirButton screen={screenData} blockId="blk_conteneurs_synchroniser" />
        <AirButton screen={screenData} blockId="blk_conteneurs_ajouter" />
      </View>
    </ScreenShell>
  );
}
