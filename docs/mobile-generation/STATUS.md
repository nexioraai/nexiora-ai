...
| P-002 Sandbox | ✅ | 🟡 **BANC E1-E5 EXÉCUTÉ** (2026-08-28, `benchmarks/sandbox-bench/synthese-P-002.md`, < 1 $/provider sur crédits) — E1 pipeline réel `npm ci→tsc→expo export` **les 2 3/3 VERTS** (Modal ~28 s vs E2B ~63 s) · E2 cache : Modal propre ~16 %, E2B instable (exit -1 en réutilisation, PROPRE en sandbox fraîche E1) · E3 egress : **tous BLOQUÉS des 2 côtés** (allowlist domaine supportée par les 2) · E4 aucun secret des 2 · E5 0 orphelin/20 des 2. **Recommandation Claude : E2B primaire (Firecracker, priorité #1 isolation), Modal second très proche et viable** — **arbitrage P-002 = décision propriétaire** |
