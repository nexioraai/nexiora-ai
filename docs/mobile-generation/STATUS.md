...
| P-002 Sandbox | ✅ | 🟡 **PRÉFLIGHTS VERTS** (2026-08-28) — E2B (cold ~440 ms) ET Modal (cold ~4,1 s) : smoke création→commande déterministe `deribfy-42`→teardown PROUVÉ par listing (0 résiduel), secrets hors dépôt 600. **Egress natif CONFIRMÉ par les SDK des DEUX finalistes** (E2B `allowInternetAccess`/`denyOut`/`updateNetwork` ; Modal `blockNetwork`/`outboundCidrAllowlist`/`outboundDomainAllowlist`). Reste : banc comparatif E1-E5 puis arbitrage P-002. Fly/Vercel non retenus pour le banc (dossier) |
