...
vi.mock('@/lib/supabase-admin', () => ({
  supabaseAdmin: {
    from: vi.fn(() => {
      const c: any = {};
      const self = () => c;
      c.select = self; c.eq = self; c.in = self; c.gte = self; c.order = self; c.limit = self;
...
function req(token?: string) {
  return new Request('https://x.test/api/cron/catalog-suggest', {
    headers: token ? { authorization: 'Bearer ' + token } : {},
  }) as any;
}

describe('GET /api/cron/catalog-suggest — authentification (fail-closed)', () => {
