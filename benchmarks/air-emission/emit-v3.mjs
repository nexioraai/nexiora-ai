...
const surfaceEnveloppe = () => {
  const faits = [
    ...
    ["liveData", "DES DONNÉES VIVANTES D'UNE SOURCE DISTANTE", "\`sourceKind: \\"remote\\"\` déclaré (+ \`sourceIntegrationId\`, \`sourceDomain\`) — mais le moteur ne CONSOMME pas encore de source distante"],
...
31. DONNÉES VIVANTES — DÉCLARER UNE PROVENANCE N'EST PAS PROUVER UNE VIVACITÉ. Un dataset PEUT déclarer d'où viendraient ses données : \`sourceKind:"remote"\` + \`sourceIntegrationId\` + \`sourceDomain\` (+ \`sourceRefreshSeconds\` optionnel) — intégration EXISTANTE et domaine PRÉSENT dans \`network.allowedDomains\`, sinon refus. Mais le moteur ne consomme pas encore de source distante (\`liveData\` ❌) : tout besoin de « temps réel », « en direct » ou de données vivantes se déclare \`unexpressible\` en citant \`liveData\` — le classer \`satisfied\` est le mensonge exact que la règle 30 interdit. Sans \`sourceKind\`, un dataset reste amorcé à la compilation : c'est le comportement historique, et il ne prétend rien.
